<?php
    function getFriendsList ($type, $page)
    {
        ...
        if (!mysqli_errno($mysqli))   // проверка корректности запроса и его результат
        {
            ...
            if ($type == 'by_id')
                $sql = "SELECT * FROM friends;"; // выборка записей из БД, сортировка по id
            else
                $sql = "SELECT * FROM friends ORDER BY surname;"; // выборка записей из БД, сортировка по фамилии
                                                    //  НЕ РАБОТАЛО ПОТОМУ ЧТО БЫЛ НЕПРАВИЛЬНЫЙ ЗАПРОС, Я УБРАЛА ЧАСТИ С LIMIT, ИХ НАДО БУДЕТ ДОБАВИТЬ В ПРАВИЛЬНОМ ВИДЕ
            $sql_res = mysqli_query($mysqli, $sql);

            $ret = '<table>';   // начало таблицы БД
            ...
            {
                $ret .= '<div id="pages">';
                for ($i=0; $i<$TOTAL; $i++) // цикл для выведения ссылок на все страницы пагинации
                    if ($i != $page)
                        $ret .= '<a href="?p=viewer&pg='.$i.'">'.($i+1).'</a>'; // если это НЕ текущая страница то отобразить как ссылку
                    else
